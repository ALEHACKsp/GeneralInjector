...
int NumberCompare( LPCTSTR pszNumber1, LPCTSTR pszNumber2 )
{
	...
	const int iNumber1 = _tstoi( pszNumber1 );
	const int iNumber2 = _tstoi( pszNumber2 );
...
{
	const int iYear1 = _tstoi( strDate1.Mid( 6, 4 ) );
	const int iYear2 = _tstoi( strDate2.Mid( 6, 4 ) );
	...
	const int iMonth1 = _tstoi( strDate1.Mid( 3, 2 ) );
	const int iMonth2 = _tstoi( strDate2.Mid( 3, 2 ) );
	...
	const int iDay1 = _tstoi( strDate1.Mid( 0, 2 ) );
	const int iDay2 = _tstoi( strDate2.Mid( 0, 2 ) );
...
void CSortListCtrl::Sort( int iColumn, BOOL bAscending )
{
	...
	VERIFY( SortItems( CompareFunction, reinterpret_cast<ULONG_PTR>( this ) ) );
}
...
BOOL CSortListCtrl::SetTextArray( int iItem, LPTSTR* arrpsz )
{
	...
	pid->arrpsz = arrpsz;
	return CListCtrl::SetItemData( iItem, reinterpret_cast<ULONG_PTR>( pid ) );
}
